...
#pragma warning disable CA1305 // Specify IFormatProvider
    ...
    public class SignatureFixup : Fixup
    {
        ...
        private static uint GetSize(BinaryReader binr)
        {
            if (binr != null)
            {
        ...
        private RSACryptoServiceProvider DecodeRSAPrivateKey(byte[] privkey)
        {
            ...
            try
            {
                ...
                {
                    throw new PeachException("Wierd ASN.1");
                }
                twobytes = binr.ReadUInt16();
                ...
                if (bt != 0x02)
                {// integer
                    throw new PeachException("Modulus not an int " + bt);
                }
                ...
                if (bt != 0x02)
                {// integer
                    throw new PeachException("Exp not an int " + bt);
                }
                elems = GetSize(binr);
                ...
                if (bt != 0x02)
                {// integer
                    throw new PeachException("D not an int " + bt);
                }
                elems = GetSize(binr);
                if ((elems & 1) == 1) { elems--; bt = binr.ReadByte(); } // remove leading zeros
                ...
                if (bt != 0x02)
                {// integer
                    throw new PeachException("P not an int " + bt);
                }
                elems = GetSize(binr);
                if ((elems & 1) == 1) { elems--; bt = binr.ReadByte(); } // remove leading zeros
                ...
                if (bt != 0x02)
                {// integer
                    throw new PeachException("Q not an int " + bt);
                }
                elems = GetSize(binr);
                if ((elems & 1) == 1) { elems--; bt = binr.ReadByte(); } // remove leading zeros
                ...
                if (bt != 0x02)
                {// integer
                    throw new PeachException("dP not an int " + bt);
                }
                elems = GetSize(binr);
                if ((elems & 1) == 1) { elems--; bt = binr.ReadByte(); } // remove leading zeros
                ...
                if (bt != 0x02)
                {// integer
                    throw new PeachException("dQ not an int " + bt);
                }
                elems = GetSize(binr);
                if ((elems & 1) == 1) { elems--; bt = binr.ReadByte(); } // remove leading zeros
                ...
                if (bt != 0x02)
                {// integer
                    throw new PeachException("Qinv not an int " + bt);
                }
                elems = GetSize(binr);
                if ((elems & 1) == 1) { elems--; bt = binr.ReadByte(); } // remove leading zeros
            ...
            {
                throw new PeachException("Create RSA error " + e.Message);
            }
            finally
            {
                binr.Close();
        ...
        private ECDsaCng DecodeECCPrivateKey(byte[] privkey)
        {
            byte[] X, Y, D;

            // ---------  Set up stream to decode the asn.1 encoded ECC private key  ------
            MemoryStream mem = new MemoryStream(privkey);
            BinaryReader binr = new BinaryReader(mem);    //wrap Memory Stream with BinaryReader for easy reading
            byte bt;
            ushort twobytes;
            ...
            try
            {
                ...
                if (bt != 0x04)
                {// integer
                    throw new PeachException("D not an octet string " + bt);
                }
                elems = GetSize(binr);
                E = binr.ReadBytes((int)elems);
                bt = binr.ReadByte();
            ...
            : base(parent, args, "ref")
        {
            ...
            else
            {
                ...
                {
                    if ((Key.Value[0] == '-') && (Key.Value[1] == '-') && (Key.Value[2] == '-') && (Key.Value[2] == '-'))
                    { // PEM
                        string text = System.Text.Encoding.UTF8.GetString(Key.Value);
                        byte[] base64EncodedBytes;
                        // plausibility check
                        if (text.Contains("-----BEGIN RSA PRIVATE KEY-----") && text.Contains("-----END RSA PRIVATE KEY-----"))
                            try
                            {
                                string keytext = text.Substring(33, text.Length - 33 - 31);            // cut off header and footer
                                base64EncodedBytes = System.Convert.FromBase64String(keytext);
                                RSAalg = DecodeRSAPrivateKey(base64EncodedBytes);
                            }
                            catch (Exception c1)
                            {
                                throw new PeachException("Cannot convert RSA PEM private key, error " + c1.Message);
                            }

                        // plausibility check
                        if (text.Contains("-----BEGIN ECC PRIVATE KEY-----") && text.Contains("-----END ECC PRIVATE KEY-----"))
                            ...
                            {
                                throw new PeachException("Cannot convert ECC PEM private key, error " + c1.Message);
                            }
                    }
                    else
                    { // try PKCS-8 format
